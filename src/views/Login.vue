	...
	methods: {
		isRedirected() {
			...
			if(error){
				if(error === "redirect_uri_mismatch"){
					...
					this.showModal("알림", "허가되지 않은 앱을 사용중입니다", this.moveTo);
				}else{
		...
		checkLoggedIn() {
			if(this.isAuthenticated()){
				...
				window.location.replace(this.github.authentication +
					"?client_id=" + this.github.cId +
					"&redirect_uri=" + escape(this.github.redirectUrl) +
					"&scope=" + escape(this.github.scope) +
					"&state=" + state);
			}
		},
